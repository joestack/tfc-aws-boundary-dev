...
locals {
  ...
  controller_ips    = element(aws_instance.server.*.private_ip, var.controller_desired_capacity)
  #controller_ips    = [for n in range(var.controller_desired_capacity) : format("%v", element(aws_instance.server.*.private_ip, n))]
